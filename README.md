...
# Dependencies
R (≥3.5.0), [data.table](https://cran.r-project.org/web/packages/data.table/index.html), [pracma](https://cran.r-project.org/web/packages/pracma/index.html), [XML](https://cran.r-project.org/web/packages/XML/index.html), [rvest](https://cran.r-project.org/web/packages/rvest/index.html), [RCurl](https://cran.r-project.org/web/packages/RCurl/index.html), [curl](https://cran.r-project.org/web/packages/curl/index.html), [purrr](https://cran.r-project.org/web/packages/purrr/index.html), [devtools](https://cran.r-project.org/web/packages/devtools/index.html), [svMisc](https://cran.r-project.org/web/packages/svMisc/index.html), [threddscrawler](https://github.com/BigelowLab/threddscrawler), [obpgcrawler](https://github.com/BigelowLab/obpgcrawler), [ncdf4](https://cran.r-project.org/web/packages/ncdf4/index.html), [raster](https://cran.r-project.org/web/packages/raster/index.html), [maps](https://cran.r-project.org/web/packages/maps/index.html), [mapdata](https://cran.r-project.org/web/packages/mapdata/index.html), [lubridate](https://cran.r-project.org/web/packages/lubridate/index.html), [lattice](https://cran.r-project.org/web/packages/lattice/index.html), [zoo](https://cran.r-project.org/web/packages/zoo/index.html), [RColorBrewer](https://cran.r-project.org/web/packages/RColorBrewer/index.html), [ggplot2](https://cran.r-project.org/web/packages/ggplot2/index.html), [ggmap](https://cran.r-project.org/web/packages/ggmap/index.html), [**rgdal v1.4-8 or older**](https://cran.r-project.org/web/packages/rgdal/index.html), [rgeos](https://cran.r-project.org/web/packages/rgeos/index.html), [sp](https://cran.r-project.org/web/packages/sp/index.html), [sf](https://cran.r-project.org/web/packages/sf/index.html), [broom](https://cran.r-project.org/web/packages/broom/index.html), [dplyr](https://cran.r-project.org/web/packages/dplyr/index.html), [reshape2](https://cran.r-project.org/web/packages/reshape2/index.html), [EnvStats](https://cran.r-project.org/web/packages/EnvStats/index.html), [gtools](https://cran.r-project.org/web/packages/gtools/index.html), [httr](https://cran.r-project.org/web/packages/httr/index.html).

Any missing dependencies should install automatically when first using a given ClimMap function. Otherwise, these can be installed manually via `install.packages()`. Please **note** that only `rgdal` package **v1.4-8 or older** are supported due to the current incompatibility of some functions with **PROJ6** and **GDAL3**.

# Getting started
1. Install both [R](https://cran.r-project.org/mirrors.html) and [RStudio](https://www.rstudio.com/products/rstudio/download/).
2. Download the latest ClimMap Toolkit release from [this link](https://drive.google.com/file/d/1XAEcbGg-2_GwzQXVF2M05I_B7eZ_JghE/view?usp=sharing) and unpack the .ZIP archive into a directory of your choice. Examples herein use **D:/** as a directory. Note that the repository only contains the [source code](https://github.com/Deniz-Koseoglu/ClimMap_Toolkit/blob/master/ClimMap_Toolkit_v091.R) and [example vignette](https://github.com/Deniz-Koseoglu/ClimMap_Toolkit/blob/master/ClimMap_Toolkit_v091_EXAMPLES.R) .R files!
...
| **plot_cols** |A character list of length 7 that specifies the colour palettes used for maps and summary plots. The **first** and **fifth to sixth** elements are multi-colour palettes (length>1) specifying, respectively, the polygon fill colours, heat map fill when `plot_type` is `"heat"` or `"heat_rank"`, and quartile colours when `plot_extras` includes `"qrt"`. The following preset colour palettes are available: "SIC", "SIC_heat", "SIC_anom", "MODIS", "ODV", "ODV_bathy", "Alter_bathy", "RYG". Elements **two to four** are single colours (length=1) controlling the colours of missing values, land, and coastline/polygon outlines, respectively. The **seventh element** is of length 3 and controls the `"heat_rank"`-specific colours in the format `c(standard colour, record low, record high)`.|
| **plot_type** |Character specifying the type of summary plot to create. One of: "point" (plot mean values per polygon as points), "bar" (...as bars), "heat" (...as a heat map), "heat_rank" (...as a heat map with record low and record high values highlighted; **useful for interannual `sat_data`**). The latter two are only supported when `facet_plots="summarise"`.|
...
| **plot_opts** |Numeric vector of length 3. Determines additional options for summary plots. The **first element** specifies the angle by which variable/polygon labels are tilted. The **second** and **third** elements respectively control the degree of vertical justification and outline width when `plot_type="bar"`.|
...
| **plot_oob** |Should observations in `sat_data` which were Out of Bounds (i.e. not assignable to any polygon within `poly_list`) be included averaged in summary plots? Defaults to `FALSE`.|
...
3. The `clim_plot` function has been deprecated, at least for now.
