#CLIMAP TOOLKIT v0.9.1 (Jun 2020)
...
#Install and load the required libraries 
list.of.packages <- c("devtools", "threddscrawler", "obpgcrawler", "httr", "XML", "rvest", "RCurl", "curl", "purrr", "devtools", "svMisc")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
...
if(any(repository %in% c("SeaWifs", "MODIS_A", "CEDA_MO", "CRU4")) & any(is.na(c(usrname, pword)))) {
  stop("CEDA Archive, MODIS Aqua, SeaWifs data require user credentials. Please provide a username and password!")
}

if(repository=="OC_CCI") {
  print("Setting default username and password for the OC_CCI (v4) dataset...")
  usrname <- "oc-cci-data"
  pword <- "ELaiWai8ae"
}
...
for(i in seq_along(year)) {
  ...
  if(repository=="SeaWifs") {
    ...
    down_url <- "https://oceandata.sci.gsfc.nasa.gov/ob/getfile/"
    ...
    if(!isTRUE(opt_down)) {
      ...
      for(j in seq_along(url_complete)) {
        ...
        #NEW SYSTEM WITH EARTHDATA LOGIN
        if(isTRUE(suppr_msgs)) {
        invisible(httr::GET(url = url_complete[j], 
            authenticate(usrname, pword),
            write_disk(path = paste0(res_dir, "/Data/", data_type,"/", frequency, "/", dirquery_unlist[j]), overwrite=TRUE)))
        } else if(!isTRUE(suppr_msgs)) {
          httr::GET(url = url_complete[j], 
              authenticate(usrname, pword),
              write_disk(path = paste0(res_dir, "/Data/", data_type,"/", frequency, "/", dirquery_unlist[j]), overwrite=TRUE))
        }
        #OLD SYSTEM WITHOUT AUTHENTICATION
        #download.file(url_complete[j], paste0(res_dir, "/Data/", data_type,"/", frequency, "/", dirquery_unlist[j]), quiet=suppr_msgs, mode=down_mode)
        }
      
    } else if(isTRUE(opt_down)) {
      stop("Parallel downloads are currently not supported for MODIS Aqua and SeaWifs data! Change opt_down to FALSE.")
    #require(parallel)
    #cl <- makeCluster(dcores)
    ...
    #NEW SYSTEM WITH EARTHDATA LOGIN
    #if(isTRUE(suppr_msgs)) {
    # invisible(capture.output(clusterMap(cl, GET, url=url_complete, 
    #                                    authenticate(usrname, pword), write_disk(path = paste0(res_dir, "/Data/", data_type,"/", frequency, "/", dirquery_unlist), overwrite=TRUE), .scheduling="dynamic")))
    #} else if(!isTRUE(suppr_msgs)) {
    #  clusterMap(cl, GET, url=url_complete, 
    #             authenticate(usrname, pword), write_disk(path = paste0(res_dir, "/Data/", data_type,"/", frequency, "/", dirquery_unlist), overwrite=TRUE), .scheduling="dynamic")
    #}
...
clim_summary <- function(remove_miss="all", frequency="daily", repository="NSIDC", hemisphere="north", data_type="Chla", nc_path, year_rng="all", month_rng="all", day_rng=NULL, var_names="default", 
  ...
  #Install and load the required libraries
  list.of.packages <- c("devtools", "purrr", "ncdf4", "raster", "maps", "mapdata", "lubridate", "ggmap", "rgdal", "sp", "sf", "chron",
                        "lattice", "svMisc", "zoo", "data.table")
  
  new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
  if(length(new.packages[-which(new.packages=="rgdal")])) install.packages(new.packages[-which(new.packages=="rgdal")])
  
  if(!any(new.packages %in% "rgdal") & !grepl("^1.4|^1.3|^1.2", packageVersion("rgdal"))) devtools::install_version("rgdal",
                                                                                                              version = "1.4-8",
                                                                                                              repos = "http://cran.us.r-project.org")
  if(any(new.packages %in% "rgdal")) devtools::install_version("rgdal",
                                                               version = "1.4-8",
                                                               repos = "http://cran.us.r-project.org")
...
clim_region <- function(core_dir, poly_path, poly_type="shp_file", poly_list="all", poly_pat="\\.shp$", separ=",",
  ...
  #Install and load the required libraries
  list.of.packages <- c("devtools", "data.table", "RColorBrewer", "ggplot2", "ggmap", "raster", "rgeos", "sp", "sf", "rgdal", "broom", "dplyr", "reshape2")
  new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
  if(length(new.packages[-which(new.packages=="rgdal")])) install.packages(new.packages[-which(new.packages=="rgdal")])
  ...
  if(coast_res[1]!="none" & exists("total_colines_coord.old")) {
    ...
    proj_bbox <- as.numeric(st_bbox(total_coline_graticules))
  ...
  #In cases where points were not detected for all polygons
  if(length(plot_cols[[1]][1:length(unique(which_poly[,"name"]))]) < length(unique(polygon_vec[["name"]]))) {
    unique_poly_detected <- length(unique(which_poly[,"name"]))
    unique_poly_total <- length(unique(polygon_vec[["name"]]))
    poly_length_diff <- unique_poly_total - unique_poly_detected
    
    regplt_cf_scale <- c(plot_cols[[1]][1:unique_poly_detected-1], 
                         rep(plot_cols[[2]], poly_length_diff))
  }
  
  
  if(any(bar_varlabs %in% "default")) {
    colour_sc <- scale_colour_manual(values=c(plot_cols[[1]][1:length(unique(which_poly[,"name"]))-1], plot_cols[[2]]))
    fill_sc_base <- scale_fill_manual(values=c(regplt_cf_scale, plot_cols[[2]]))
    ...
    colour_sc <- scale_colour_manual(values=c(plot_cols[[1]][1:length(unique(which_poly[,"name"]))-1], plot_cols[[2]]), labels=bar_varlabs[which(unique(poly_plainvec[,"name"]) %in% as.character(unique(which_poly[,"name"])))])
    fill_sc_base <- scale_fill_manual(values=c(regplt_cf_scale, plot_cols[[2]]), labels=bar_varlabs[which(unique(poly_plainvec[,"name"]) %in% as.character(unique(which_poly[,"name"])))])
  }
  ...
  summary_finalres <- summary_finalres[order(summary_finalres[,"Region"]),]
  ...
  if(plot_type!="none") {
    ...
    if(plot_by=="polygon") {
      ...
      if(facet_plots==TRUE & plot_type!="heat" | facet_plots==TRUE & plot_type!="heat_rank") {
        ...
        summary_stats[,"Timevar"] <- factor(summary_stats[,"Timevar"], levels=unique(summary_stats[,"Timevar"]))
...
clim_locate <- function(sat_data, point_data, ex_method="bilinear", coord_vars=c("Longitude", "Latitude"), sat_vars=sat_data[,!colnames(sat_data) %in% coord_vars],
  ...
  #Install and load the required libraries
  list.of.packages <- c("devtools", "data.table", "raster", "rgeos", "sp", "sf", "rgdal", "broom", "dplyr")
